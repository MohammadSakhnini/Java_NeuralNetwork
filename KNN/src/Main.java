public class Main {
	...
	double[][] inputs = { { 0, 0 }, { 0, 1 }, { 1, 0 }, { 1, 1 } };// eingabe
	var inputLayer = new InputLayer(inputs, 3, WeightGenerator.He, ActivationFunction.Identity);
	var hiddenLayer = new HiddenLayer(3, 1, WeightGenerator.He, ActivationFunction.Sigmoid);
	NeuralNetwork nn = new NeuralNetwork(0.01);
	nn.addLayer(inputLayer);
	nn.addLayer(hiddenLayer);
	var outputLayer = nn.addOutputLayer(1, ActivationFunction.Tanh);
	nn.train(target, 100);
